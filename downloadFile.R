...
start <- Sys.time()
blogs <- read_lines(file("final/en_US/en_US.blogs.txt"), skip = 0, n_max = 400)
set.seed(485)
...
start <- Sys.time()
news <- read_lines(file("final/en_US/en_US.news.txt"), skip = 0, n_max = 400)
set.seed(123)
...
start <- Sys.time()
twitter <- read_lines(file("final/en_US/en_US.twitter.txt"), skip = 0, n_max = 400)
set.seed(987)
